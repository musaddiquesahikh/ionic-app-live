...
@Component({
  selector: 'app-sales-register-details',
  templateUrl: './sales-register-details.page.html',
  styleUrls: ['./sales-register-details.page.scss'],
})
export class SalesRegisterDetailsPage implements OnInit {
  sales: any[];
  company: any = []
  item: any = {}
  companyId: any;
  ...
  private async generateItems() {
    ...
    if (this.pagination.next_page) {
      let header = this.api.getHeader();
      this.api.searchsalesdetails(s, this.item,header).subscribe(async (response: any) => {
  ...
  submit() {
    ...
    let s = ''
    let header = this.api.getHeader();
    this.api.searchsalesdetails(s, this.e,header).subscribe(async (response: any) => {
...
search(ev:any){
  ...
  getSales(ev) {
    ...
    this.e.start_date = this.item.start_date
    let header = this.api.getHeader();
    this.api.searchsalesdetails(this.event, this.e,header).subscribe(async (response: any) => {
      this.PartyDataS = response.data
      if (s && s.trim() === '') {
        this.salesData = []
