...
@Component({
  ...
  templateUrl: './category-wise-purchase.page.html',
  styleUrls: ['./category-wise-purchase.page.scss'],
})
export class CategoryWisePurchasePage implements OnInit {
  ...
  private async generateItems() {
    ...
    if (this.pagination.next_page) {
      ...
      this.api.categoryPurchase(s, this.item,header).subscribe(async (response: any) => {
  ...
  submit() {
    ...
    let s = ''
    let header = this.api.getHeader();
    this.api.categoryPurchase(s, this.e,header).subscribe(async (response: any) => {
