...
@Component({
  ...
  templateUrl: './edit-payment-in.page.html',
  styleUrls: ['./edit-payment-in.page.scss'],
})
export class EditPaymentInPage implements OnInit {
  payment: any = {};
  array: any = []
  company: any = []
  result3: any = []
  ...
  constructor(public modalCtrl: ModalController, public api: ApiService, public router: Router,
    public location: Location, public toastController: ToastController, private translate: TranslateService) { }
  ...
  getData() {
    ...
    this.api.getParticularPayment(this.data.id, { company: this.data.company }).subscribe((response: any) => {
      ...
      console.log("this.payment.payment_for", this.payment.payment_for);

      this.payment.invoice_list = JSON.parse(this.payment.invoice_list)
      console.log("this.payment.invoice_list", this.payment.invoice_list);
      console.log("this.payment.Party_name", this.payment.party_name);

      if (this.payment.party_name) {
  ...
  editPaymentIn() {
    ...
    if (this.payment)
      this.api.editPaymentInDemo(this.data.id, this.payment, header).subscribe(async (response: any) => {
