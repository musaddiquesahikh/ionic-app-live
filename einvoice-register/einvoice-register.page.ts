...
import { ToastController, ModalController, InfiniteScrollCustomEvent } from '@ionic/angular';
import { TranslateService } from '@ngx-translate/core';
import { ApiService } from '../api.service';

@Component({
  selector: 'app-einvoice-register',
  templateUrl: './einvoice-register.page.html',
  styleUrls: ['./einvoice-register.page.scss'],
})
export class EInvoiceRegisterPage implements OnInit {
  pagination: any;
  page_number: any = 1;
  Itemdata: any[];
  ...
  private async generateItems() {
    ...
    if (this.pagination.next_page) {
      ...
      this.api.EinvoiceReg(s, this.item1,header).subscribe(async (response: any) => {
  ...
  submit() {
    ...
    let s = ''
    let header = this.api.getHeader();
    this.api.EinvoiceReg(s, this.e,header).subscribe(async (response: any) => {
