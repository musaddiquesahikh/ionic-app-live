...
@Component({
  selector: 'app-view-cash-transaction',
  templateUrl: './view-cash-transaction.page.html',
  styleUrls: ['./view-cash-transaction.page.scss'],
})
export class ViewCashTransactionPage implements OnInit {
  company: any = []
  data1: any = {}
  cashData: any = []
  fileName = 'cTransaction.xlsx';
  length: any
  constructor(public api: ApiService, public router: Router, public modalCtrl: ModalController, public toastController: ToastController,
  ...
  viewTransaction() {
    ...
    this.api.viewCashLedger(this.data1, header).subscribe(async (response: any) => {
      ...
      if (this.cashData) {
        this.length = this.cashData.length
      }
